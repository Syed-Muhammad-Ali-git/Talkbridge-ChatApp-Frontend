...
import { Phone, Video, Search, MoreVertical, Paperclip, Smile, Send, Mic, CheckCheck } from 'lucide-react';
...
const ChatWindow = ({ activeChat, messages, messageInput, setMessageInput, onSendMessage }: ChatWindowProps) => {
  ...
  if (!activeChat) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-bg-main p-8 text-center animate-in fade-in duration-500">
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white relative overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
      {/* Header */}
      <div className="h-[76px] px-6 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h3 className="font-bold text-text-primary leading-tight">{activeChat.name}</h3>
            <p className={`text-[11px] font-medium ${activeChat.status === 'online' ? 'text-online' : 'text-text-secondary'}`}>
