  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        { icon: Moon, label: "Dark Mode", description: darkMode ? "On" : "Off", hasToggle: true, key: 'darkMode' },
              ...
              <div className="space-y-1">
                {section.items.map((item, itemIdx) => (
                  <button
                    key={itemIdx}
                    onClick={() => {
                      if (item.key === 'darkMode') setDarkMode(!darkMode);
                    }}
                  ...
                  >
                    ...
                    {item.hasToggle ? (
                      <div className={`w-10 h-5 rounded-full relative transition-colors ${darkMode ? 'bg-primary' : 'bg-gray-200'}`}>
                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full shadow-sm transition-all ${darkMode ? 'left-6' : 'left-1'}`} />
                      </div>
                    ) : (
